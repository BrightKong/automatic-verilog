...
"input/output/inout port adjust{{{2
function PortAdjust(line,cmts)
    ...
    "extract {{{3
    let prefix = repeat(' ',4)
    let port = matchstr(line,'\v<input>|<output>|<inout>')  "input/output/inout
    let type = matchstr(line,'\v<wire>|<reg>|<logic>')      "wire/reg/logic
    "[31:0]
...
function JoinStandaloneBegin()
    let idx = 1
    while idx < line('$')
        let line = getline(idx)
        let next_line = getline(idx + 1)
        let code = substitute(substitute(line,'\/\*.*\*\/','',''),'\/\/.*','','')
        let next_code = substitute(substitute(next_line,'\/\*.*\*\/','',''),'\/\/.*','','')
        if code !~ '\<begin\>\s*$'
                    \ && next_code =~ '^\s*begin\s*$'
                    \ && code =~ '^\s*\(always\(_ff\|_comb\|_latch\)\?\>\|if\>\|else\>\|for\>\)'
            call setline(idx,substitute(line,'\s*$','','').' begin')
            execute ':'.(idx + 1).'d'
        else
            let idx = idx + 1
        endif
...
function AlignAlways()
    ...
    while idx < line('$')
        ...
        if line =~ '^\s*always\(_ff\|_comb\|_latch\)\?\>'
            let tab = 1
...
"Main Function: Format Adjust for systemverilog {{{1
function FormatAdjust()
    ...
    call JoinStandaloneBegin()
    call AlignAlways()
    "}}}2

    "align input/output/inout, reg/wire/logic, localparam/parameter, delete redundant space {{{2
    let idx = 0
    while idx < line('$')
        let idx = idx + 1
        let line = getline(idx)
        ...
        "input/output/inout port adjust {{{3
        if line =~ '\v^\s*(<input>|<output>|<inout>)'
            "adjust current line
        ...
        "reg/wire/logic adjust {{{3
        if line =~ '\v^\s*(<reg>|<wire>|<logic>)'
